...
const AllOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      setError("Server error while fetching orders. Please check your network or server.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-6">
      <div className="max-w-7xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md flex flex-col lg:flex-row gap-6">
        ...
        <div
          className={
            selectedOrder ? "flex-1 overflow-x-auto lg:w-2/3" : "w-full overflow-x-auto"
          }
        >
                ...
                orders.map((order, idx) => (
                  <tr
                    ...
                    className={`border-t dark:border-gray-700 cursor-pointer transition-colors duration-200
                      ${selectedOrder?._id === order._id ? "bg-gray-300 dark:bg-gray-700" : "hover:bg-gray-100 dark:hover:bg-gray-700/50"}
                    `}
                    onClick={() => setSelectedOrder(order)}
                  >
                    <td className="px-4 py-2">{idx + 1}</td>
                    <td className="px-4 py-2">{order.user?.name || "N/A"}</td>
                    <td className="px-4 py-2">{formatPrice(order.offerPrice || order.totalAmount)}</td>
        ...
        {selectedOrder && (
          <div className="w-full lg:w-1/3 bg-gray-50 dark:bg-gray-700 rounded-md p-6 shadow-lg relative">
            <h3 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white border-b pb-2 border-gray-200 dark:border-gray-600">
              Order Details (#<span className="font-mono">{selectedOrder._id.slice(-6)}</span>)
            </h3>
            <div className="text-sm text-gray-800 dark:text-gray-200 space-y-3 max-h-[80vh] overflow-y-auto custom-scrollbar pr-2">
              ...
              <p>
                <strong>Total Amount:</strong> {formatPrice(selectedOrder.offerPrice || selectedOrder.totalAmount)}
              </p>
              <p>
              ...
              <p>
                ...
                <span className="font-semibold text-indigo-600 dark:text-indigo-300">{selectedOrder.status}</span>
              </p>
              ...
              <div className="mt-4 border-t pt-3 border-gray-200 dark:border-gray-600">
                ...
                <p>
                  {selectedOrder.shippingAddress?.address || "N/A"}, {selectedOrder.shippingAddress?.landmark ? `${selectedOrder.shippingAddress.landmark}, ` : ''}
                ...
                <div className="space-y-3">
                  {selectedOrder.items && selectedOrder.items.length > 0 ? (
                    selectedOrder.items.map((item) => (
                      ...
                      >
                        <img
                          src={item.product?.image || "https://placehold.co/50x50/e0e0e0/ffffff?text=No+Image"} // Fallback image
                    ...
                    <p className="text-gray-600 dark:text-gray-300">No items in this order.</p>
                  )}
                </div>
              </div>
