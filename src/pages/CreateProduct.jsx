...
const CreateProduct = () => {
  ...
  const uploadToCloudinary = async (image) => {
    ...
    data.append("cloud_name", "dtjngyozu");

    const res = await fetch("https://api.cloudinary.com/v1_1/dtjngyozu/image/upload", {
      method: "POST",
      body: data,
    });
    ...
    <div className="h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center px-4">
      <div className="bg-white dark:bg-gray-800 w-full max-w-lg rounded-xl shadow-md p-8 overflow-auto">
        <h2 className="text-xl font-semibold text-gray-800 dark:text-white text-center mb-4">
          Create Product in: <span className="text-green-500">{category?.name || "..."}</span>
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Color selection */}
          <div>
            <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Available Colors</label>
            <div className="flex flex-wrap gap-3">
              {COLORS.map((color) => (
                <label key={color} className="flex items-center space-x-2 text-sm text-gray-700 dark:text-gray-300">
                  <input
                    type="checkbox"
          ...
          {/* Size selection */}
          <div>
            <label className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Available Sizes</label>
            <div className="flex flex-wrap gap-3">
              {SIZES.map((size) => (
                <label key={size} className="flex items-center space-x-2 text-sm text-gray-700 dark:text-gray-300">
                  <input
                    type="checkbox"
